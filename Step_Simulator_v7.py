...
def step():
    ...
    if Single_Motor == True:
        ...
        if turning == 'u_turn':
            right_wheel_direction = 'backward'
            left_wheel_direction = 'forward'
            steps_right = (pi * (dist_btwn_wheels / 2)) / 2
            steps_left = (pi * (dist_btwn_wheels / 2)) / 2
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    running = False
                    close = True
            if angle_right > int(steps_right*-1):
                angle_right -= 1
            if angle_left > int(steps_left*-1):
                angle_left -= 1
            screen.fill((255, 255, 255))
            ...
            if Single_Motor == True:
                if orientation == 'top':
                    screen.blit(robot, ((col * constant), (row * constant)-Single_Motor_Pixels))
                if orientation == 'left':
                    screen.blit(robot, ((col * constant)-Single_Motor_Pixels, (row * constant)))
                if orientation == 'right':
                    screen.blit(robot, ((col * constant)+Single_Motor_Pixels, (row * constant)))
                if orientation == 'bottom':
                    screen.blit(robot, ((col * constant), (row * constant)+Single_Motor_Pixels))
            ...
            w,h = wheel.get_size()
            rotate(screen,wheel,(892,455),(w/2,h/2),angle_right)
            rotate(screen, wheel, (892, 185), (w / 2, h / 2), angle_left)
